...
------------------------------------------------------------------------------------------------ */
// Passed
const addValues = (arr) => arr.reduce((sum, value) => sum + value, 0);
...
const addPurchases = (arr) => {
  return arr.reduce((total, price, idx) => total + arr[idx].purchasePrice, 0);
};
...
const returnNames = (arr) =>
  arr.reduce((acc, person) => [...acc, person.name], []);
...
//     return acc + arr[arr.length - 1 - acc.length];
//   });
// };
...
const reversedString = (str) => {
  let arr = Array.from(str);
  return arr.reduce((acc, l) => l.concat(acc));
};
...
const countNumberOfChildren = (arr) => {
  ...
  arr.reduce((acc, fam) => {
    if (!fam.children) {
      ...
      fam.children.forEach((i) => (count += 1));
    }
  }, []);
  return count;
};
