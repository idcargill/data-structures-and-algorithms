...
const longestString = (arr) => {
  ...
  arr.forEach((str, idx) => {
    if (str.length > arr[longestIndex].length) {
      longestIndex = idx;
    }
  });
  return longestIndex;
...
CHALLENGE 2
PASS
Write a function named firstLetters that takes in an array of strings and returns an 
array containing only the first letter of each string.
...
const standardizePhoneNumbers = (arr) => {
  let regex = /[()-\s]/gi;
  return arr.map((num) => num.replace(regex, ""));
};
...
// PASS
const allHappy = (arr) => {
  const checked = arr.map((str) => str.includes(":)"));
...
const findEvery = (arr, target) => {
  const checked = arr.map((str) => str.includes(target));
  return checked.includes(false) ? false : true;
};
...
Write a function named unenrollBrook that takes in a two-dimensional array, where each array represents one 
course's roster and is an array of strings of the names of the people in that course.

Return a two-dimensional array with the same roster, but where anyone whose name includes Brook is removed 
from every course.
...
const unenrollBrook = (arr) => {
  return arr.map((subArr) =>
    subArr.filter((person) => !person.includes("Brook"))
  );
};
